 ...
 *
 * Prevents Snazzy from running on WordPress versions prior to 4.7,
...
/**
 * Prevent switching to Snazzy on old versions of WordPress.
 *
...
function themebeans_upgrade_notice() {
	$message = sprintf( esc_html__( 'Snazzy requires at least WordPress version 4.7. You are running version %s. Please upgrade and try again.', 'snazzy' ), $GLOBALS['wp_version'] );
...
function themebeans_customize() {
	wp_die(
		sprintf( esc_html__( 'Snazzy requires at least WordPress version 4.7. You are running version %s. Please upgrade and try again.', 'snazzy' ), esc_html( $GLOBALS['wp_version'] ) ), '', array(
			'back_link' => true,
		)
	);
}
...
function themebeans_preview() {
	if ( isset( $_GET['preview'] ) ) {
		wp_die( sprintf( esc_html__( 'Snazzy requires at least WordPress version 4.7. You are running version %s. Please upgrade and try again.', 'snazzy' ), esc_html( $GLOBALS['wp_version'] ) ) );
	}
}
