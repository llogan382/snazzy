...
if ( ! class_exists( 'ThemeBeans_License' ) ) :
	...
	class ThemeBeans_License {
		...
		public function __construct( $config = array(), $strings = array() ) {
			...
			// Development filter.
			add_action( 'themebeans_remove_license_activation_notice', array( $this, 'disable_activation_notice_for_development' ) );
		}
		...
		public function license_control( $wp_customize ) {
			...
			$wp_customize->add_section(
				'license_key', array(
					'title' => esc_html__( 'License Key', 'snazzy' ),
				)
			);
			...
			$description = sprintf( esc_html__( 'Enter a license key to enable remote updates and access theme support for %s.', 'snazzy' ), $this->item_name );

			$url = 'https://kb.themebeans.com/article/14-license-activation-guide';

			$tooltip = esc_html__( 'Need help?', 'snazzy' );

			if ( $this->status() ) {

				if ( 'valid' === $this->status() ) {

					$description = esc_html__( 'Awesome! Your site is connected to ThemeBeans and ready for remote updates, and support if you need it.', 'snazzy' );

				} elseif ( 'expired' === $this->status() ) {

					/* translators: 1. theme name, 2. expiration date */
					$description = sprintf( esc_html__( 'Your license for %1$s expired on %2$s and is no longer connected to ThemeBeans. Please renew your license to activate remote updates and support.', 'snazzy' ), $this->item_name, $this->expiration() );

					// Set the tooltip URL to use the renewal link instead.
					$url = $this->get_renewal_link();

					$tooltip = esc_html__( 'Renew License', 'snazzy' );
			...
			$wp_customize->add_control(
				new ThemeBeans_License_Control(
					$wp_customize, 'themebeans_license[key]', array(
						...
						/* translators: theme name */
						'label'       => sprintf( esc_html__( '%s License', 'snazzy' ), $this->item_name ),
		...
		public function add_schedules( $schedules = array() ) {
			...
			$schedules['weekly'] = array(
				'interval' => 604800,
				'display'  => esc_html__( 'Once Weekly', 'snazzy' ),
			);
		...
		/**
		 * Output a notification of a missing license.
		 *
		 * Use the following filter to disable the activation notice.
		 *
		 * function themebeans_remove_license_notice() {
		 *       return true;
		 * }
		 * add_action( 'themebeans_remove_license_activation_notice', 'themebeans_remove_license_notice' );
		 */
		public function activate_license_notice() {
			...
			// Continue if the notice is not disabled.
			if ( apply_filters( 'themebeans_remove_license_activation_notice', false ) ) {
				return;
			}

			// Look up the license key and it's status.
			$key    = $this->key();
			$status = $this->status();

			// If we have a key and status, return.
			if ( $key && $status ) {
				return;
			}
			...
			/* translators: %1$s: start link, %2$s: close link, %3$s: theme name, %4$s: help icon & link */
			echo '<div class="notice notice-warning"><p>' . sprintf( esc_html__( '%1$sActivate your license key%2$s for %3$s to enable remote updates and theme support. %4$s', 'snazzy' ), '<a href="' . esc_url( admin_url( '/customize.php?autofocus[section]=license_key' ) ) . '">', '</a>', esc_html( $this->item_name ), '<a style="text-decoration: none; float: right;" href="' . esc_url( $url ) . '" target="_blank"><span class="dashicons dashicons-editor-help"></span></a>' ) . '</p></div>';
		}

		/**
		 * Disable the activation notice for development zones.
		 */
		public function disable_activation_notice_for_development() {

			if ( ! defined( 'THEMEBEANS_DEVELOPMENT' ) ) {
				return;
			}

			// Continue if the current user can manage options.
			if ( THEMEBEANS_DEVELOPMENT ) {
				return true;
			}
		}

		/**
		 * Output a notification of an expired license.
		 */
		public function expired_notice() {
			...
			// Look up the license's expiration date.
			$status     = $this->status();
			$expiration = $this->expiration();

			// Has this license expired? If not, return early.
			if ( 'expired' !== $status ) {
				return;
			}

			$expired_on = date_i18n( get_option( 'date_format' ), strtotime( $expiration ) );

			/* translators: %1$s: theme name, %2$s: expiration date, %3$s: link start, %4$s: link close */
			echo '<div class="error"><p>' . sprintf( esc_html__( 'Your license for %1$s expired on %2$s. %3$sRenew your license or club membership%4$s to activate remote updates and support.', 'snazzy' ), esc_html( $this->item_name ), esc_html( $expired_on ), '<a href="' . esc_url( $this->get_renewal_link() ) . '" target="_blank">', '</a>' ) . '</p></div>';
		}
		...
		public function options_page() {
			...
			add_theme_page( esc_html__( 'Theme License', 'snazzy' ), esc_html__( 'Theme License', 'snazzy' ) . $notifcation, 'manage_options', 'themebeans-license-key', '__return_null' );
		}
		...
		public function activate_license() {
			...
			if ( ! isset( $response->license ) ) {
				$message = esc_html__( 'An error occurred, please try again.', 'snazzy' );
			} else {
					...
					switch ( $response->error ) {
						...
						case 'expired':
							$message = sprintf(
								...
								esc_html__( 'Your license expired on %1$s. %2$sClick here to renew &rarr;%3$s', 'snazzy' ),
						...
						case 'revoked':
							$message = esc_html__( 'Your license key has been disabled.', 'snazzy' );
							break;

						case 'missing':
							$message = esc_html__( 'Invalid license.', 'snazzy' );
							break;

						case 'invalid':
						case 'site_inactive':
							$message = esc_html__( 'Your license is not active for this URL.', 'snazzy' );
							break;

						case 'item_name_mismatch':
							$message = esc_html__( 'This appears to be an invalid license key.', 'snazzy' );
							break;

						case 'no_activations_left':
							$message = esc_html__( 'Your license key has reached its activation limit.', 'snazzy' );
							break;
			...
			if ( ! isset( $response->license ) ) {
				...
				if ( false === $response->success ) {
					...
					switch ( $response->error ) {
						...
						case 'expired':
							$message = sprintf(
								...
								esc_html__( 'Your license key expired on %s.', 'snazzy' ),
								date_i18n( get_option( 'date_format' ), strtotime( $response->expires, current_time( 'timestamp' ) ) )
							);
							break;
						...
						case 'missing':
							$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'snazzy' );
							break;
					...
					if ( 'valid' === $response->license ) {
						$message = esc_html__( 'Your theme is activated! Remote updates and theme support are enabled.', 'snazzy' );
						$success = true;
					}
				}
			}
