...
class ThemeBeans_License_Control extends WP_Customize_Control {
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'themebeans-' . $control . '-control', get_parent_theme_file_uri( 'inc/admin/controls/assets/css' . $dir . $control . SNAZZY_ASSET_SUFFIX . '.css' ), false, '3.2.8', 'all' );
		wp_enqueue_script( 'themebeans-' . $control . '-control', get_parent_theme_file_uri( 'inc/admin/controls/assets/js' . $dir . $control . SNAZZY_ASSET_SUFFIX . '.js' ), array( 'jquery' ), '3.2.8', true );
	...
	protected function content_template() {
		...
		<div id="theme-license-form" class="license-form">
			...
			<input type="submit" name="themebeans-license" id="themebeans-activate-license" value="<?php esc_attr_e( 'Activate', 'snazzy' ); ?>" class="button-secondary button {{ data.visibility }}">
			<input type="submit" name="themebeans-deactivate-license" id="themebeans-deactivate-license" value="<?php esc_attr_e( 'Deactivate', 'snazzy' ); ?>" class="button-secondary button {{ data.visibility }}">
		...
		<ul id="theme-license-info" class="{{ data.visibility }}">
			<li><strong><?php esc_html_e( 'Status:', 'snazzy' ); ?></strong> <span id="theme-license-status">{{ data.status }}</span></li>
		</ul>
