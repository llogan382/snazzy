	...
	foreach ( $meta_box['fields'] as $field ) {
		...
		switch ( $field['type'] ) {
			...
			<?php
				...
				$thumbnail_output = '';
				$button_text      = ( $meta ) ? __( 'Edit Gallery', 'snazzy' ) : $field['std'];
			if ( $meta ) {
				$field['std']     = __( 'Edit Gallery', 'snazzy' );
				$thumbs           = explode( ',', $meta );
				$thumbnail_output = '';
			...
			case 'checkbox':
                ...
                <input type="checkbox" id="' . $field['id'] . '" name="bean_meta[' . $field['id'] . ']" value="on"' . $val . ' /><span>' . __( 'Yes, please do', 'snazzy' ) . '</span>';
				echo '</td>';
				break;
		}
