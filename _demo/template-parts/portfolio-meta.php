...
<div class="project-meta">
    ...
    <?php if ($portfolio_date == 'on') { ?>
        <h6 class="published">
            <?php esc_html_e( 'Date: ', 'snazzy' ); ?><span><?php the_time('M Y') ?></span>
        </h6>
    <?php } ?>

    <?php if ($portfolio_role) { ?>
        <h6 class="role">
            <?php esc_html_e( 'Role: ', 'snazzy' ); ?><span><?php echo esc_html( $portfolio_role ); ?></span>
        </h6>
    <?php } ?>

    <?php if ($portfolio_client) { ?>
        <h6 class="client">
            <?php esc_html_e( 'Client: ', 'snazzy' ); ?>
            <span>
    ...
    <?php if ($portfolio_url && !$portfolio_client ) { ?>
        <h6 class="url">
            <?php esc_html_e( 'URL: ', 'snazzy' ); ?><span><a href="<?php echo esc_url($portfolio_url); ?>" target="_blank"><?php echo esc_html( $portfolio_url_clean ); ?></a></span>
        </h6>
    <?php } ?>

    <?php if ($portfolio_views == 'on') { // DISPLAY VIEWS ?>
        <h6 class="views">
            <?php esc_html_e( 'Views: ', 'snazzy' ); ?><span><?php echo esc_html(snazzy_get_post_views(get_the_ID())); ?></span>
        </h6>
    <?php } ?>
