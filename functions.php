...
if ( ! defined( '__PREFIX_DEBUG' ) ) :
	/**
	 * Check to see if development mode is active.
	 * If set to false, the theme will load un-minified assets.
	 */
	define( '__PREFIX_DEBUG', true );
endif;

if ( ! defined( '__PREFIX_ASSET_SUFFIX' ) ) :
	/**
	 * If not set to true, let's serve minified .css and .js assets.
	 * Don't modify this, unless you know what you're doing!
	 */
	if ( ! defined( '__PREFIX_DEBUG' ) || true === __PREFIX_DEBUG ) {
		define( '__PREFIX_ASSET_SUFFIX', null );
	} else {
		define( '__PREFIX_ASSET_SUFFIX', '.min' );
	}
endif;
...
if ( ! function_exists( 'snazzy_setup' ) ) :
	...
	function snazzy_setup() {
		 ...
		 */
		add_editor_style( array( 'assets/css/editor' . __PREFIX_ASSET_SUFFIX . '.css', snazzy_fonts_url() ) );
		...
		$starter_content = array(
			...
			'options'     => array(
				...
				'page_on_front'   => '{{home}}',
				'page_for_posts'  => '{{blog}}',
				'blogname'        => _x( 'Snazzy', 'Theme starter content', '@@textdomain' ),
				'blogdescription' => _x( 'A snazzy portfolio WordPress theme by ThemeBeans', 'Theme starter content', '@@textdomain' ),
			),
			...
			'posts'       => array(
				...
				'home' => array(
					'post_type'    => 'page',
					'post_title'   => _x( 'Portfolio', 'Theme starter content', '@@textdomain' ),
					'post_content' => '',
				),
			),
			...
			'theme_mods'  => array(
				...
				'contact_button'        => _x( 'Send Message', 'Theme starter content', '@@textdomain' ),
			),
...
function snazzy_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Theme Sidebar', '@@textdomain' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Appears site-wide in the header block.', '@@textdomain' ),
...
function snazzy_scripts() {
	...
	// Load theme styles.
	if ( is_child_theme() ) {
		wp_enqueue_style( 'snazzy-style', get_parent_theme_file_uri( '/style' . __PREFIX_ASSET_SUFFIX . '.css' ) );
		wp_enqueue_style( 'snazzy-child-style', get_theme_file_uri( '/style.css' ), false, '@@pkg.version', 'all' );
	} else {
		wp_enqueue_style( 'snazzy-style', get_theme_file_uri( '/style' . __PREFIX_ASSET_SUFFIX . '.css' ) );
	}

	/**
	 * Now let's check the same for the scripts.
	 */
	if ( SCRIPT_DEBUG || __PREFIX_DEBUG ) {
		// Vendor scripts.
...
if ( ! function_exists( 'snazzy_protected_form' ) ) :
	...
	function snazzy_protected_form() {
		...
		<label for="' . $label . '">' . esc_html__( 'Password:', '@@textdomain' ) . ' </label><input name="post_password" placeholder="' . esc_html__( 'Enter password here & press enter...', '@@textdomain' ) . '" type="password" placeholder=""/><input type="submit" name="Submit" value="' . esc_attr__( 'Submit', '@@textdomain' ) . '" />
		</form>
		';
...
?>
					...
					<div class="comment-metadata">
						...
						<?php printf( wp_kses( __( '<cite class="fn">%s</cite> ', '@@textdomain' ), $allowed_html_array ), get_comment_author_link() ); ?>
...
?>
							<?php if ( $comment->comment_approved == '0' ) : ?>
								<span class="comment-awaiting-moderation"><?php esc_html_e( 'Awaiting moderation', '@@textdomain' ); ?></span>
							<?php endif; ?>
						</span>
					</div>
...
if ( ! function_exists( 'snazzy_comment_submit_button' ) ) :
	...
	function snazzy_comment_submit_button( $button ) {
		$button = '<input name="submit" type="submit" class="form-submit" value="' . __( 'Submit', '@@textdomain' ) . '" />';
		return $button;
	}
