...
if ( ! function_exists( 'snazzy_comment_nav' ) ) :
	...
	function snazzy_comment_nav() {
		...
		<div class="nav-links">
			<?php
			if ( $prev_link = get_previous_comments_link( esc_html__( 'Older Comments', '@@textdomain' ) ) ) :
				printf( '<div class="nav-previous">%s</div>', $prev_link );
				endif;

			if ( $next_link = get_next_comments_link( esc_html__( 'Newer Comments', '@@textdomain' ) ) ) :
	...
	function snazzy_entry_meta() {
		...
		if ( ! is_singular() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			...
			echo '<span class="comments-link">';
				comments_popup_link( sprintf( wp_kses( __( 'Leave a comment<span class="screen-reader-text"> on %s</span>', '@@textdomain' ), $allowed_html_array ), get_the_title() ) );
			echo '</span>';
		}
	}
endif;
...
if ( ! function_exists( 'snazzy_entry_taxonomies' ) ) :
	...
	function snazzy_entry_taxonomies() {
		$categories_list = get_the_category_list( esc_html_x( ', ', 'Used between list items, there is a space after the comma.', '@@textdomain' ) );
		if ( $categories_list && snazzy_categorized_blog() ) {
			printf(
				'<span class="cat-links"><span class="screen-reader-text">%1$s </span>%2$s</span>',
				esc_html_x( 'Categories', 'Used before category names.', '@@textdomain' ),
				$categories_list
			);
		}

		$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'Used between list items, there is a space after the comma.', '@@textdomain' ) );
		if ( $tags_list ) {
			printf(
				'<span class="tags-links"><span class="screen-reader-text">%1$s </span>%2$s</span>',
				esc_html_x( 'Tags', 'Used before tag names.', '@@textdomain' ),
				$tags_list
			);
		}
	}
...
if ( ! function_exists( 'snazzy_site_archive' ) ) :
	...
	function snazzy_site_archive() {
		if ( is_singular() && 'page' == get_post_type() ) {
			...
			printf( '<p>%s</p>', esc_html( 'Monthly', '@@textdomain' ) );
			printf(
			...
			printf( '<p>%s</p>', esc_html( 'Yearly', '@@textdomain' ) );
			printf(
				'<ul>%s</ul>', wp_get_archives(
					array(
						'type'  => 'yearly',
			...
			printf( '<p>%s</p>', esc_html( 'Categories', '@@textdomain' ) );
			printf(
