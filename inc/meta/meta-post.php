<?php
/**
 * The file is for creating the blog post type meta.
 * Meta output is defined on the page editor.
 * Corresponding meta functions are located in framework/metaboxes.php
 *
 * @package     @@pkg.name
 * @link        @@pkg.theme_uri
 * @author      @@pkg.author
 * @copyright   @@pkg.copyright
 * @license     @@pkg.license
 */

add_action('add_meta_boxes', 'bean_metabox_post');
function bean_metabox_post(){

$prefix = '_bean_';



/*===================================================================*/
/*  VIDEO POST FORMAT SETTINGS
/*===================================================================*/
$meta_box = array(
	'id' => 'bean-meta-box-video',
	'title' =>  esc_html__('Video Settings', '@@textdomain'),
	'page' => 'post',
	'context' => 'normal',
	'priority' => 'high',
	'fields' => array(
        array(
            'name' => esc_html__('Lightbox Embed:', '@@textdomain'),
            'desc' => esc_html__('Insert a embeded URL for a video lightbox.', '@@textdomain'),
            'id' => $prefix . 'post_embed_url',
            'type' => 'text',
            'std' => ''
            ),
        array(
            'name' => esc_html__('Embed:', '@@textdomain'),
            'desc' => esc_html__('Alternatively, insert an embed code.', '@@textdomain'),
            'id' => $prefix . 'post_embed',
            'type' => 'textarea',
            'std' => ''
            ),
	)
);
bean_add_meta_box( $meta_box );
} // END function bean_metabox_post()