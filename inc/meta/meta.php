	...
	foreach ( $meta_box['fields'] as $field ) {
		...
		switch ( $field['type'] ) {
				...
				jQuery(function($) {
					...
					$('#<?php echo esc_js( $field['id'] ); ?>_button').on('click', function(e) {
						...
						frame.toolbar.get('view').set({
							insert: {
								style: 'primary',
								text: '<?php esc_html_e( 'Insert', 'snazzy' ); ?>',
							...
							insert: {
								...
								click: function() {
									var models = frame.state().get('selection'),
										...
										this.el.innerHTML = '<?php esc_html_e( 'Saving...', 'snazzy' ); ?>';
			...
			?>
				...
				jQuery(function($) {
					...
					$('#bean_images_upload').on('click', function(e) {
						...
						//FIRST FRAME
						var options = {
							title: '<?php esc_html_e( 'Create Gallery', 'snazzy' ); ?>',
							state: 'gallery-edit',
							frame: 'post',
							selection: selection
						};
						//IF FRAME IS THERE
						if( frame || selection ) {
							options['title'] = '<?php esc_html_e( 'Edit Gallery', 'snazzy' ); ?>';
						}

						frame = wp.media(options).open();

						// CUSTOMIZE VIEWS
						frame.menu.get('view').get('cancel').el.innerHTML = '<?php esc_html_e( 'Cancel', 'snazzy' ); ?>';
						frame.menu.get('view').get('gallery-edit').el.innerHTML = '<?php esc_html_e( 'Edit Gallery', 'snazzy' ); ?>';
						...
						function overrideGalleryInsert() {
							frame.toolbar.get('view').set({
								insert: {
									style: 'primary',
									text: '<?php esc_html_e( 'Save Gallery', 'snazzy' ); ?>',
			...
			<?php
				...
				$thumbnail_output = '';
				$button_text      = ( $meta ) ? __( 'Edit Gallery', '@@textdomain' ) : $field['std'];
			if ( $meta ) {
				$field['std']     = __( 'Edit Gallery', '@@textdomain' );
				$thumbs           = explode( ',', $meta );
				$thumbnail_output = '';
			...
			case 'checkbox':
                ...
                <input type="checkbox" id="' . $field['id'] . '" name="bean_meta[' . $field['id'] . ']" value="on"' . $val . ' /><span>' . __( 'Yes, please do', '@@textdomain' ) . '</span>';
				echo '</td>';
				break;
		}
