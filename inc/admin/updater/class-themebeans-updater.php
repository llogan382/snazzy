...
if ( ! class_exists( 'ThemeBeans_Updater' ) ) :
	...
	class ThemeBeans_Updater {
		...
		public function update_nag() {
			...
			$update_onclick = ' onclick="if ( confirm(\'' . esc_attr__( 'Updating this theme will lose any customizations you have made. "Cancel" to stop, "OK" to update.', '@@textdomain' ) . '\') ) {return true;}return false;"';
			...
			if ( version_compare( $this->version, $api_response->new_version, '<' ) ) {
				...
				if ( 'expired' === $status ) {
					echo '<div class="notice notice-warning">';
						printf(
							__( '<p>%1$s <a href="%2$s" title="%2$s" target="blank">v%3$s</a> has been released, although a valid license is required to enable updates.</p>', '@@textdomain' ),
					...
					echo '<div id="update-nag">';
						printf(
							__( '<strong>%1$s %2$s</strong> is available. <a href="%3$s" title="%4$s" target="blank">Check out what\'s new</a> or <a href="%5$s" %6$s>update now</a>', '@@textdomain' ),
							esc_html( $theme->get( 'Name' ) ),
							esc_html( $api_response->new_version ),
							'http://demo.themebeans.com/wp-content/themes/' . esc_attr( $this->theme_slug ) . '/changelog.txt',
							esc_html( $theme->get( 'Name' ) ),
