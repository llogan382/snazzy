	...
	class ThemeBeans_License {
		...
		public function license_control( $wp_customize ) {
			...
			$wp_customize->add_section(
				'license_key', array(
					'title' => esc_html__( 'License Key', '@@textdomain' ),
				)
			);
			...
			$description = sprintf( esc_html__( 'Enter a license key to enable remote updates and access theme support for %s.', '@@textdomain' ), $this->item_name );

			$url = 'https://kb.themebeans.com/article/14-license-activation-guide';

			$tooltip = esc_html__( 'Need help?', '@@textdomain' );

			if ( $this->status() ) {

				if ( 'valid' === $this->status() ) {

					$description = esc_html__( 'Awesome! Your site is connected to ThemeBeans and ready for remote updates, and support if you need it.', '@@textdomain' );

				} elseif ( 'expired' === $this->status() ) {

					/* translators: 1. theme name, 2. expiration date */
					$description = sprintf( esc_html__( 'Your license for %1$s expired on %2$s and is no longer connected to ThemeBeans. Please renew your license to activate remote updates and support.', '@@textdomain' ), $this->item_name, $this->expiration() );

					// Set the tooltip URL to use the renewal link instead.
					$url = $this->get_renewal_link();

					$tooltip = esc_html__( 'Renew License', '@@textdomain' );
			...
			$wp_customize->add_control(
				new ThemeBeans_License_Control(
					$wp_customize, 'themebeans_license[key]', array(
						...
						/* translators: theme name */
						'label'       => sprintf( esc_html__( '%s License', '@@textdomain' ), $this->item_name ),
		...
		public function activate_license_notice() {
			...
			echo '<div class="notice notice-warning"><p>' . sprintf( esc_html__( '%1$sActivate your license key%2$s for %3$s to enable remote updates and theme support. %4$s', '@@textdomain' ), '<a href="' . esc_url( admin_url( '/customize.php?autofocus[section]=license_key' ) ) . '">', '</a>', esc_html( $this->item_name ), '<a style="text-decoration: none; float: right;" href="' . esc_url( $url ) . '" target="_blank"><span class="dashicons dashicons-editor-help"></span></a>' ) . '</p></div>';
		}
		...
		public function expired_notice() {
			...
			echo '<div class="error"><p>' . sprintf( esc_html__( 'Your license for %1$s expired on %2$s. %3$sRenew your license or club membership%4$s to activate remote updates and support.', '@@textdomain' ), esc_html( $this->item_name ), esc_html( $expired_on ), '<a href="' . esc_url( $this->get_renewal_link() ) . '" target="_blank">', '</a>' ) . '</p></div>';
		}
		...
		public function options_page() {
			...
			add_theme_page( esc_html__( 'Theme License', '@@textdomain' ), esc_html__( 'Theme License', '@@textdomain' ) . $notifcation, 'manage_options', 'themebeans-license-key', '__return_null' );
		}
					...
					switch ( $response->error ) {
						...
						case 'expired':
							$message = sprintf(
								...
								esc_html__( 'Your license expired on %1$s. %2$sClick here to renew &rarr;%3$s', '@@textdomain' ),
			...
			if ( ! isset( $response->license ) ) {
				...
				if ( false === $response->success ) {
					...
					if ( 'valid' === $response->license ) {
						$message = esc_html__( 'Your theme is activated! Remote updates and theme support are enabled.', '@@textdomain' );
						$success = true;
					}
				}
			}
